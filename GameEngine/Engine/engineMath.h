...
#pragma warning(pop)
    ...
    {
        Vector3 retVal(vec.x, vec.y, vec.z);
        retVal.Normalize();
        return retVal;
    }
    ...
    inline static bool IsCloseEnuf(const Vector3& a, const Vector3& b, Vector3 epsilon = Vector3(0.001f, 0.001f, 0.001f))
    {
        return fabsf(a.x - b.x) <= epsilon.x * Math::Max(Math::Max(1.0f, fabsf(a.x)), fabsf(b.x))
            && fabsf(a.y - b.y) <= epsilon.y * Math::Max(Math::Max(1.0f, fabsf(a.y)), fabsf(b.y))
            && fabsf(a.z - b.z) <= epsilon.z * Math::Max(Math::Max(1.0f, fabsf(a.z)), fabsf(b.z));
    }
...
#pragma warning(pop)
    ...
    {
        Vector4 retVal(vec.x, vec.y, vec.z, vec.w);
        retVal.Normalize();
        return retVal;
    }
...
public:
    ...
    {
        return Vector3(this->mat[0][0], this->mat[0][1], this->mat[0][2]);
    }

    // Get the Y axis of the matrix (left)
    Vector3 GetYAxis() const
    {
        return Vector3(this->mat[1][0], this->mat[1][1], this->mat[1][2]);
    }

    // Get the Z axis of the matrix (up)
    Vector3 GetZAxis() const
    {
        return Vector3(this->mat[2][0], this->mat[2][1], this->mat[2][2]);
    }
