...
public:
    ...
    void SetRenderTarget(ID3D11RenderTargetView* renderTarget, ID3D11DepthStencilView* pDepthStencilView);
    void ClearRenderTarget(const Color4 &clearColor); // Legacy way to clear rendertargets. Calls ClearRenderTargets()
    void ClearRenderTarget(const Color4 &clearColor, UINT viewIndex); 
    void SetRenderTargets(UINT numViews, ID3D11RenderTargetView* const* renderTargets, ID3D11DepthStencilView* pDepthStencilView);
    void ClearRenderTargets(const Color4& clearColor);
    void EndFrame();
    ...
    ID3D11DepthStencilView* GetStencilView() const { return mDepthStencilView; }
...
private:
    ...
    ID3D11RenderTargetView* mCurrentRenderTargets[D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT];
